...
export class FormResponseService {
  ...
  getFormResponseSummaries(
    ...
    this.http.get<FormResponseSummaries>(url, {
      params: {
  ...
  getIndividualFormResponseByFormResponseId(formId: string, formResponseId: string, onComplete: (res: ResponseIndividualRes) => void) {
  ...
  getIndividualFormResponseByFormResponsePage(formId: string, formResponsePage: number, onComplete: (res: ResponseIndividualRes) => void) {

    const url = `http://localhost:9094/api/v1/forms/${formId}/responses`

    this.http.get<ResponseIndividualRes>(url, {params: {page: formResponsePage}}).subscribe(res => {
      onComplete(res)
    })
  }
