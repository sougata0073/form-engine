...
import { EditFormMultipleChoiceOption } from '../edit-form-multiple-choice/edit-form-multiple-choice-option/edit-form-multiple-choice-option';
import { MultipleChoiceOption } from '../../../../type/multiple-choice-option';
import { MatRadioButton, MatRadioGroup } from '@angular/material/radio';
import { OnlyMultipleChoiceGridAddUpdateReq } from '../../../../model/edit-form/question/addreq/multiple-choice-grid-add-req';
import {
  MultipleChoiceGridUpdateReq,
  OnlyMultipleChoiceGridUpdateReq,
} from '../../../../model/edit-form/question/updatereq/multiple-choice-grid-update-req';
...
export class EditFormMultipleChoiceGrid
  extends EditFormQuestionComponent<MultipleChoiceGridRes, OnlyMultipleChoiceGridUpdateReq>
  implements OnInit
{
  ...
  override onQuestionInputChange(change: SimpleChange<MultipleChoiceGridRes>): void {
    this.rows.set(
      this.question().rows.map((r) => ({
        id: r.id,
        option: r.row,
  ...
  protected addRow() {
    ...
    this.updateQuestion.emit({
      req: {
        ...
        updateFields: ['rows' satisfies keyof MultipleChoiceGridUpdateReq],
      },
      updateType: 'CRITICAL',
    });
  }
  ...
  protected onRowTextChange(row: DropdownOption) {
    this.rows.update((val) => val.map((v) => (v.id === row.id ? { ...v, option: row.option } : v)));
    this.emiCanSaveHasError();
  ...
  protected addColumn() {
    if (this.columns().length >= 20) {
      this.dialog.open(SimpleDialog, {
        data: SimpleDialog.configure('Error', 'Can not add more than 20 columns', 'Close'),
      });
      return;
    }
    ...
    this.updateQuestion.emit({
      req: {
        ...
        updateFields: ['columns' satisfies keyof MultipleChoiceGridUpdateReq],
      },
      updateType: 'CRITICAL',
    });
  }
  ...
  protected removeColumn(columnId: string) {
    if (this.columns().length <= 1) {
      this.dialog.open(SimpleDialog, {
        data: SimpleDialog.configure('Error', 'At least 1 column is required', 'Close'),
      });
      return;
    }
    this.columns.update((val) => {
