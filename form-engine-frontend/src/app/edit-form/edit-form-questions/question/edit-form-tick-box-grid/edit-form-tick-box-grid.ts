...
} from '../../../../model/edit-form/question/updatereq/tick-box-grid-update-req';
...
export class EditFormTickBoxGrid
  extends EditFormQuestionComponent<TickBoxGridRes, OnlyTickBoxGridUpdateReq>
  implements OnInit
{
  ...
  override onQuestionInputChange(change: SimpleChange<TickBoxGridRes>): void {
    this.rows.set(
      this.question().rows.map((r) => ({
        id: r.id,
        option: r.row,
  ...
  protected addRow() {
    ...
    this.updateQuestion.emit({
      req: {
        ...
        updateFields: ['rows' satisfies keyof TickBoxGridUpdateReq],
      },
      updateType: 'CRITICAL',
    });
  }
  ...
  protected addColumn() {
    ...
    this.updateQuestion.emit({
      req: {
        ...
        updateFields: ['columns' satisfies keyof TickBoxGridUpdateReq],
      },
      updateType: 'CRITICAL',
    });
  }
